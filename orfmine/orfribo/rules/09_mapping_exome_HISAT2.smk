...
if config.get('rna_to_exclude'):       
	rule Mapping_Exome_Hisat2_Bowtie2:
            ...
            shell:
               "hisat2 -x {params.index_names_hisat2} --threads {threads} -k {params.multi_map}  -U {input.fastq} --un-gz {output.fastq_hisat2} -S {output.sam_hisat2} 2>> {log.hisat2_out};"
               "bowtie2 -x {params.index_names_bowtie2} --threads {threads} -k {params.multi_map} -U {output.fastq_hisat2} -S {output.sam_bowtie2} 2>> {log.bowtie2_out};" 
