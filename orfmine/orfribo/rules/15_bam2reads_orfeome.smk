...
rule Bam2Reads_ORFeome:
    ...
    output:
        reads_table= str(DATA_PROCESSING_PATH / "Bam2Reads_ORFeome" / "{sample}"/ ("{sample}_{length}/ORFeome_{length}_reads.tab"))
    log: 
      bam2read= str(LOGS_PATH / "Bam2Reads_ORFeome"  / "{sample}.{length}.bam2read.log"),
      offset_grep= str(LOGS_PATH / "Bam2Reads_ORFeome" / "{sample}.{length}.offset_grep.log")
    resources:
        mem_mb = MEM_MB
    benchmark: 
        str(BENCHMARKS_PATH / "Bam2Reads_ORFeome" / "{sample}.{length}.bam2read.benchmark.txt")
    params:
        outdir = str(DATA_PROCESSING_PATH / "Bam2Reads_ORFeome" / "{sample}" / "{sample}_{length}/"),
        sample_name = "{sample}",
        reads_length = "{length}",
