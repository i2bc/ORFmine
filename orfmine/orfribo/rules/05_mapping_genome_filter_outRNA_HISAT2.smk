...
rule index_reference_HISAT2:
    input:
         fasta= str(FASTA_PATH)
    output:
         expand(str(DATA_PROCESSING_PATH / "Mapping"/ "ORFeome"/ "Hisat2" / "Index" / "index_hisat2.{exth}.ht2"),exth=HISAT2)
    log:
         str(LOGS_PATH / "Mapping" / "ORFeome" / "Hisat2" / "Index" / "ORFeome_index.log")
    benchmark:
         str(BENCHMARKS_PATH / "Mapping" / "ORFeome" / "Hisat2" / "Index" / "ORFeome_index.benchmark.txt")
    params:
         str(DATA_PROCESSING_PATH / "Mapping"/ "ORFeome"/ "Hisat2" / "Index" / "index_hisat2")
    threads:
         THREADS_NB
    shell:
...
if config.get('rna_to_exclude'):       
	rule Mapping_ORFeome_Hisat2_Bowtie2:
	    input: 
	       fastq = str(DATA_PROCESSING_PATH / "Mapping" / "Filter_Unwanted_Sequence" / "Results" / "{sample}" / "{sample}_Unmapped.fastq.gz"),    
               index_hisat2 = expand(str(DATA_PROCESSING_PATH / "Mapping"/ "ORFeome"/ "Hisat2" / "Index" / "index_hisat2.{exth}.ht2"),exth=HISAT2),
	    ...
	    output:
	       ...
	       fastq_hisat2 = str(DATA_PROCESSING_PATH / "ORFeome_Fastq" / "{sample}" / "{sample}_Unmapped.fastq.gz")
	    params:
	       index_names_bowtie2 = str(DATA_PROCESSING_PATH / "Mapping"/ "ORFeome"/ "Bowtie2"/ "Index" / "index_bowtie2"),
	       index_names_hisat2 = str(DATA_PROCESSING_PATH / "Mapping"/ "ORFeome"/ "Hisat2" / "Index" / "index_hisat2")
	    threads: 
	       THREADS_NB
	    log:
	       hisat2_out = str(LOGS_PATH / "Mapping" / "ORFeome" / "Hisat2" / "{sample}_hisat2_mapping.log"),
	       bowtie2_out = str(LOGS_PATH / "Mapping" / "ORFeome" / "Bowtie2" / "{sample}_bowie2_mapping.log")
	    benchmark:
	       str(BENCHMARKS_PATH / "Mapping" / "ORFeome" / "Hisat2" / "{sample}_HISAT_Bowtie2_Mapping_ORFeome.benchmark.txt")
	    shell:
