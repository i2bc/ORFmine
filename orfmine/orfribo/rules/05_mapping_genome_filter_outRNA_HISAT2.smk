...
rule index_reference_HISAT2:
    input:
         fasta= str(FASTA_PATH)
    output:
         expand(str(DATA_PROCESSING_PATH / "Mapping"/ "Genome"/ "Hisat2" / "Index" / "index_hisat2.{exth}.ht2"),exth=HISAT2)
    log:
         str(LOGS_PATH / "Mapping" / "Genome" / "Hisat2" / "Index" / "Genome_index.log")
    benchmark:
         str(BENCHMARKS_PATH / "Mapping" / "Genome" / "Hisat2" / "Index" / "Genome_index.benchmark.txt")
    params:
         str(DATA_PROCESSING_PATH / "Mapping"/ "Genome"/ "Hisat2" / "Index" / "index_hisat2")
    threads:
         THREADS_NB
    shell:
...
rule Mapping_Genome_Hisat2_Bowtie2:
    input: 
        fastq = str(DATA_PROCESSING_PATH / "Mapping" / "Mapping_Unwanted_Sequence_And_Filtering" / "Results" / "{sample}" / "{sample}_Unmapped.fastq.gz"),    
        index_hisat2 = expand(str(DATA_PROCESSING_PATH / "Mapping"/ "Genome"/ "Hisat2" / "Index" / "index_hisat2.{exth}.ht2"),exth=HISAT2),
    ...
    output:
        ...
        fastq_hisat2 = str(DATA_PROCESSING_PATH / "Genome_Fastq" / "{sample}" / "{sample}_Unmapped.fastq.gz")
    params:
        index_names_bowtie2 = str(DATA_PROCESSING_PATH / "Mapping"/ "Genome"/ "Bowtie2"/ "Index" / "index_bowtie2"),
        index_names_hisat2 = str(DATA_PROCESSING_PATH / "Mapping"/ "Genome"/ "Hisat2" / "Index" / "index_hisat2")
    threads: 
        THREADS_NB
    log:
