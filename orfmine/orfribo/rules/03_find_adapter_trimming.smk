...
rule find_adapter:
     ...
     log:
        ...
        already_trimmed = str(LOGS_PATH / "Trimming" / "Adapters" / "{sample}.altready_trimmed.log")
     params:
...
rule adapt_trimming_and_Select_Reads_Lenght:
    ...
    shell:
        """
        if [ {params.trimmed} == 1 ]; then
            cutadapt -e 0.125 -j {threads} --max-n=1 -m {params.min} -M {params.max} -o {output.cut_fastq} {input.fastq} 1>> {log.cutadapt_out} 2> {log.cutadapt}
            echo "Reads are already trimmed." > {log.cutadapt_out}
        elif  [ {params.trimmed} == 0 ]; then
