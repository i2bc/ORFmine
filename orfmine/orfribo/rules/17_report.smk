    ...
    rule report_analysis:
        ...
        shell:
            ...
            if [ -f "{params.exclude}" ]; then
                python3 {params.script} --fastq {input.fastq_brut} --fastq_trimmed {input.trimmed_fastq} --unwanted_sequence {params.log_filtred_unwanted_seq} --exome {input.log_exome_hisat2} {input.log_exome_bowtie2} --genome {input.log_genome_hisat2} {input.log_genome_bowtie2} --output {output}
            else
                python3 {params.script} --fastq {input.fastq_brut} --fastq_trimmed {input.trimmed_fastq} --exome {input.log_exome_hisat2} {input.log_exome_bowtie2} --genome {input.log_genome_hisat2} {input.log_genome_bowtie2} --output {output}
            fi 
            """
...
if MAPPING_TOOL == "star":
    rule report_analysis:
        ...
        shell:
            """
            if [ -f "{params.exclude}" ]; then
                python3 {params.script} --fastq {input.fastq_brut} --fastq_trimmed {input.trimmed_fastq} --unwanted_sequence {params.log_filtred_unwanted_seq} --exome {input.log_exome_star} {input.log_exome_bowtie2} --genome {input.log_genome_star} {input.log_genome_bowtie2} --output {output}
            else
                python3 {params.script} --fastq {input.fastq_brut} --fastq_trimmed {input.trimmed_fastq} --exome {input.log_exome_star} {input.log_exome_bowtie2} --genome {input.log_genome_star} {input.log_genome_bowtie2} --output {output}
            fi 
            """
