if config.get('rna_to_exclude'):
        rule report_stats:
             input: 
                  ...
                  hisat2 = str(DATA_PROCESSING_PATH / "Mapping" / "Filter_Unwanted_Sequence" / "Results" / "{sample}"/ "{sample}_Unmapped.fastq.gz") 
             output: 
             ...
             shell:
                  ...
                  if [ "{params.tool}" == "hisat2" ]; then
                      sh {params.script} {output} {input.brut} {input.trimming} {input.hisat2} 
                  elif [ "{params.tool}" == "star" ]; then
                      sh {params.script} {output} {input.brut} {input.trimming} {input.star}
                  else
