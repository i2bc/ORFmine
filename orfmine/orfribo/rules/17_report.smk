if config.get('rna_to_exclude'):
        rule report_stats:
             input: 
                  ...
                  #hisat2 = expand(str(DATA_PROCESSING_PATH / "Mapping" / "Mapping_Unwanted_Sequence_And_Filtering" / "Results" / "{sample}"/ "{sample}_Unmapped.fastq.gz"), sample=SAMPLES)
             output: 
             ...
             shell:
                  ...
                  if [ "{params.tool}" == "hisat2" ]; then
                      sh {params.script} {output} {input.brut} {input.trimming} {params.hisat2} 
                  elif [ "{params.tool}" == "star" ]; then
                      sh {params.script} {output} {input.brut} {input.trimming} {params.star}
                  else
