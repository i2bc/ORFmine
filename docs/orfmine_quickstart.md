...
``` bash
orfold -faa /database/mapping_orf_Scer_nc.pfasta  -options HIT
```
...
<div class="admonition note">
    ...
    <p class="last">
        The option -options HIT implies that IUPred and Tango have been downloaded
and that the full installation (including IUPred and Tango) has been undertaken.
    </p>
</div>
...
as calculated by HCA [1], IUPred [2][3][4] and TANGO [5][6][7], respectively
...
of globular proteins taken from Mészáros et al. [6]. It takes as input the table generatd by ORFold that must be located in the /workdir/orfold/ directory of the container.
...
9. Papadopoulos, C., Arbes, H., Chevrollier, N., Blanchet, S., Cornu, D., Roginski, P., Rabier, C., Atia, S., Lespinet, O., Namy, O., Lopes, A. (submitted).
