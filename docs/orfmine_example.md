...
``` bash
orfget --fna /database/Scer.fna --gff /database/mapping_orf_Scer.gff --features_include nc --singularity
```
...
``` bash
orfget --fna /database/Scer.fna --gff /database/Scer.gff --features_include CDS --singularity
```

An example of this output is available in the ORFmine/examples/database/ directory.
...
foldability and the disorder and aggregation propensities. Before running Orfold, you need to install the IUPRED2 and Tango tools, and you must also provide their paths via the command line. of all the amino acid sequences potentially encoded in the noncoding ORFs of yeast. 

``` bash
orfold --faa /database/mapping_orf_Scer_nc.pfasta  --options H --path_tango /path/to/executable/tango --path_iupred /path/to/executable/iupred --singularity 
```

with **--options** H to indicate that we want to estimate the foldability (H for HCA). 


ORFold generates one table in the current directory or in the output directory defined using **--out** argument. A copy of the output can be found in ORFmine/examples/workdir/orfold directory. The table contains for each input
...
``` bash
orfold --faa /database/mapping_orf_Scer_nc.pfasta --options H --gff /database/mapping_orf_Scer.gff --path_tango /path/to/executable/tango --path_iupred /path/to/executable/iupred --singularity 
```


In addition to the score table, ORFold has generated a new gff file containing for each ORF its fold potential. The output gff file is stored output directory and can be subsequently uploaded on a genome viewer.
...
### Input files
ORFribo relies on the fastq file(s). an example of such file is available in the ORFmine/examples/fastq folder

* /fastq/SRR1520313_17031088.fastq.gz


and 4 additional files. Examples of such files are available in the ORFmine/examples/database/ folder.
...
``` bash
orfribo --fna /examples/database/Scer.fna --gff /examples/database/Scer.gff --gff-intergenic /examples/database/mapping_orf_Scer.gff --fastq /fastq/ --not-trimmed --singularity 

```

Be careful to have enough memory on your computer/cluster using the parameter **--ram** for you analysis as a lack of RAM would lead the pipeline to crash without giving any error message. So if the program stops without any obvious reason this might be the reason why, and one should restart the pipeline with more resources.
...
ORFdate generates two outputs in csv format. Examples of these outputs can be found in the ORFmine/exmaples/workdir/orfdate/ folder.
...
ORFdate generates also intermediate files (blast databases and blast outputs). Examples of such outputs can be found in the ORFmine/examples/workdir/orfdate/ directory.
