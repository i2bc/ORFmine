...
After following this procedure, you could juste copy/paste all of the commands below to test them directly. We suppose you're using singularity, if you are using singularity, replace `--singularity` par `--docker` 
...
```bash
orfplot --tab /workdir/orfold/mapping_orf_Scer_nc.tab --labels  "Yeast noncoding ORFs"
```
...
``` bash
orfribo --fna /examples/database/Scer.fna --gff /examples/database/Scer.gff --gff-intergenic /examples/database/mapping_orf_Scer.gff --fastq /fastq/ --not-trimmed
...
```
orfdate --target Scer -tree /database/orfdate_inputs/Saccharomyces_species.nwk --mapping /database/orfdate_inputs/names.csv
```
